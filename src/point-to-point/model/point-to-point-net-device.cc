...
//	}
	if ((nodeId.id_level == 0) && ((iif - 1) == dstId.id_pod)) { // iif starts from 1, but podId start from 0.
		...
		return false;
	} else if ((iif > Port_num / 2) && (nodeId.id_switch != dstId.id_switch)
			&& (nodeId.id_level == 2)) {
		return false;
	} else if ((iif <= Port_num / 2) && (nodeId.id_pod == dstId.id_pod)&& (nodeId.id_level == 1)) {
		return false;
	} else
		return true;
}
...
uint32_t PointToPointNetDevice::Forwarding_FatTree(Ptr<Packet> packet,
	...
	uint32_t normalOif = NormalForwarding_FatTree(nodeId, dstId, srcId,
			turningId);
	//rerouting key equal to the dstId, turning Id, nodeId.
	std::string reRoutingKey = i2s(dstId.id_pod) + i2s(dstId.id_switch)
			...
			+ i2s(nodeId.id_pod) + i2s(nodeId.id_switch) + i2s(nodeId.id_level);
...
//		std::cout<<"ok!!"<<std::endl;
		...
		//when failure happens, we add the fixed rerouting table.
		(*reRoutingMap)["011000001"] = 1;
		(*reRoutingMap)["011000002"] = 3+ Port_num/2;
		(*reRoutingMap)["011000021"] = 2;
		(*reRoutingMap)["200000012"] = 2+Port_num/2;
		oif = (*reRoutingMap)[reRoutingKey];
//		std::cout<<"hah"<<"oif = "<<oif<<std::endl;
	}
	return oif;
}
